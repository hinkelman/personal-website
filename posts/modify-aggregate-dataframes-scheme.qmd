...
This post is part of a series on the [dataframe library](https://github.com/hinkelman/dataframe/) for Scheme (R6RS). In this post, I will contrast the `dataframe` library with functions from base R and the [`dplyr` package](https://dplyr.tidyverse.org) for modifying and aggregating dataframes.
...
[I opted not to use the term mutate in the `datafame` library because it felt too directly contradictory with the fact that dataframes are immutable. Arguably, this was a silly choice because modify is a synonym of mutate.]{.aside}
...
`dplyr` also provides `mutate_at` and `mutate_all`. [These functions have been superseded by the use of `across` within `mutate`.]{.aside}
