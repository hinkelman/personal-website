...
`generate-bar` simply draws the inputs without doing any calculations. `total-width` is measured in the number of characters. A `tick` is a single character to indicate completion.[I've chosen `=` as the completion character following the style of R's [`progress` package](https://github.com/r-lib/progress).]{.aside} The `places-string` keeps the percent completed number at 3 characters so that the total length of the bar doesn't change and distract from the intended animation effect.
