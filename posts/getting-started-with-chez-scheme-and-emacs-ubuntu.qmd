...
``` bash
curl -L -O https://github.com/cisco/ChezScheme/releases/download/v10.3.0/csv10.3.0.tar.gz
tar -xf csv10.3.0.tar.gz
cd csv10.3.0/
./configure
make
sudo make install
...
You may need to install `libncurses-dev` with `sudo apt install libncurses-dev` before building from source.
