...
The fecundity value is then looked up by name, which is safer than by position, e.g., `fecundity[["resident"]]` returns `1000`. [indexing with `[` returns the name and the value. Using `[[` returns the value only.]{.aside}
