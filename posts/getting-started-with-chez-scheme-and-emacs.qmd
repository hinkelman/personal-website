...
The `"."` indicates that Chez is looking in the current directory.[You can find the current directory by running `current-directory`.]{.aside} If you are using a project-based workflow, then you could include your dependencies in the current directory, perhaps in a `lib` folder. For a 'global' approach, I created a library directory at `/Users/username/scheme/lib` on macOS, and at `C:\scheme\lib` on Windows.
