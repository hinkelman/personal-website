...
Our example data was handled well by `csv->list`, but if your data is more unruly then you can turn to the lower-level functions in the `csv-reading` package, which involve making a `csv-reader` and then iteratively calling that reader.  
