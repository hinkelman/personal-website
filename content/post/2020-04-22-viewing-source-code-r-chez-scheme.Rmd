...
In Chez Scheme, we can view source code for exported procedures with the [inspector](https://cisco.github.io/ChezScheme/csug9.5/debug.html#./debug:s22). Let's load up my [`chez-stats` library](https://hinkelman.github.io/chez-stats/) and inspect `mean`.^[When I tried to use the inspector from the Emacs REPL, it would occassionally hang up. Not sure if that is a quirk of my system or session, but I now run `inspect` after launching Chez from the terminal.]
