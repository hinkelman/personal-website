...
The `save-rktd` function first checks if the data structure is serializable and returns an error if it is not.^[Racket’s `if` is similar to `ifelse` in R.] The `with-output-to-file` handles opening and closing a port to the file. While the port is open, the anonymous function (specified with `(lambda)`) indicates that the data should first be serialized and then written to the output port. The default argument `#:exists` is set to `'replace` because overwriting the existing file is familiar to me from `saveRDS`.
