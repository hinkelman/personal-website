...
When filling out a new roster, a dropdown menu^[I don't remember why I opted for [`pickerInput`](https://www.rdocumentation.org/packages/shinyWidgets/versions/0.4.5/topics/pickerInput) from the [shinyWidgets package](https://dreamrs.github.io/shinyWidgets/index.html) over the default `selectInput` widget.] allows for selecting from previously entered players. The dropdown is dynamically created with `renderUI()` because the contents of the dropdown depend on previous selections.
```
