---
title: Library to access Chez Scheme documentation from the REPL
...
After writing the file, I added the few lines of code needed to make [`data.ss`](https://github.com/hinkelman/chez-docs/blob/master/data.ss) a library that exports only the data. 

### `chez-docs`

`chez-docs` only exports one procedure, `doc`, which uses `case-lambda` to handle optional arguments with default values. 
