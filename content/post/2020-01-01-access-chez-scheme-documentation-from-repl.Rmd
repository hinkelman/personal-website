...
`display-launch` takes an association list, `data-selected`, returned by `data-lookup`, displays the form(s), and optionally opens a link to the relevant section of the documentation in your default browser. `display-launch` makes a system call to `open` (macOS) or `xdg-open` (Linux) and requires an internet connection if `launch?` is `#t`.
...
`open-string` returns the appropriate system call based on the value returned by `(machine-type)`. I haven't yet worked out how this should work on Windows.
