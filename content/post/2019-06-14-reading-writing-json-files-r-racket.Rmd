...
date: '2019-06-14'
slug: reading-writing-json-files-r-racket
categories:
  - R
  - Racket
tags:
...
In learning about [reading CSV files in Racket](/post/reading-csv-files-in-r-and-racket/), I have started to reconsider whether storing small(ish) datasets in CSV files is the best default behavior.^[I even used CSV files as a [database replacement](/post/dt-datatable-crud/) in my [Shiny Scorekeeper app](/project/shiny-scorekeeper/).] My default was set by primarily working in R, where reading and writing CSV files plays a central role in data analysis. When working solely in R, I expect that my old habits will die hard and CSV files will continue to play a prominent role. However, when passing small(ish) data betwee R and Racket, I think JSON might be a better alternative.

Before we jump into a bunch of little code examples, let's define some wrapper functions to make the subsequent code a bit more concise. In R, we are using the [`jsonlite` package](https://github.com/jeroen/jsonlite) and a little function to write the JSON object to file.
...
In Racket, we are using the [`json` module](https://docs.racket-lang.org/json/index.html?q=json) and have simple wrapper functions for both reading and writing JSON. A `jsexpr` is a subset of Racket values (e.g., list, hash table, etc.) that can be represented as JSON strings.
...
The data frame is the central data structure in R, which corresponds well to flat, delimited text files (in contrast to JSON). In this code chunk, we use the same [example CSV file](/post/example-data/example.csv) as in the [CSV reading post](/post/reading-csv-files-in-r-and-racket/). 
...
Racket reads the data frame as a list of [hash tables](https://docs.racket-lang.org/reference/hashtables.html?q=hasheq#%28def._%28%28quote._~23~25kernel%29._hasheq%29%29). Note that keys in the hash table are listed in alphabetical order. Also, the redundancy of the JSON format is evident for this tabular data via the repetition of the column names.
...
Racket reads the nested list as a nested hash table^[See also [previous post](/post/named-lists-hash-tables/) on the similarity of nested lists in R and nested hash tables in Racket.] rather than a list of hash tables as with the data frame. As with reading the data frame above, the keys are listed in alphabetical order.
