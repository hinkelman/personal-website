...
date: '2019-07-13'
slug: progress-bar-widget-in-r-and-racket
categories:
  - R
  - Racket
tags:
...
The `progress-bar%` class includes a single method, `set-value`, for updating both the `gauge%` and the `message%`^[`~a` is shorthand for converting a number to a string. `paste` in R implicitly converts values to strings before concatenation, but `string-append` in Racket requires that all arguments are strings.]. Interestingly, you can call the `set-value` method from within the class definition. 
