...
We loop through the rows of the unique groups and partition the dataframe.^[`dataframe-partition` will be covered in the next post of this series.] `dataframe-partition` returns two dataframes. The `keep` and `drop` dataframes contain the rows where the `filter-expr` is `#t` and `#f`, respectively. 
...
In contrast, `datframe-bind` will drop all columns not shared across the dataframes being bound.
