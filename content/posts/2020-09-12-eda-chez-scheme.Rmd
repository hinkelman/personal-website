...
`chez-stats`^[In retrospect, `read-delim` is a better fit for the `dataframe` library.] has a procedure for reading delimited text files, `read-delim`, where the default separator is a comma. 
...
The `dataframe` library includes pipeline operators taken from on an early implementation of [SRFI 197](https://srfi.schemers.org/srfi-197/srfi-197.html).^[SRFI 197 has now deviated considerably from that early syntax.] The `->` places the output of the previous expression as the first argument of the next expression. 
