...
My typical entry point to learning about Chez Scheme is the [Summary of Forms](https://cisco.github.io/ChezScheme/csug9.5/summary.html) page of the Chez Scheme User's Guide. The simple idea behind `chez-docs` is to scrape^[Scraping code is in a [different repository](https://github.com/hinkelman/chez-docs-scrape) than `chez-docs`.] the data from the Summary of Forms page and write a procedure that opens links to the documentation from the REPL. 
...
First, I needed to make a small modification to my [`csv` library](https://github.com/hinkelman/chez-stats/blob/dev/csv.ss) to read TSV files. Then, I read each TSV file, dropped the header row, and combine the two lists into a list for writing to file.
...
To read the data when `chez-docs` is loaded, we need to identify the path where the data is located. For `(import (chez-docs))` to work, the user needs to have `chez-docs.ss` and `chez-docs-data.scm` in a directory found by `(library-directories)`^[See this [blog post](/post/getting-started-with-chez-scheme-and-emacs/) for more information on library directories.]. Thus, we can loop through the list of library directories to find the file location and read the data. 
