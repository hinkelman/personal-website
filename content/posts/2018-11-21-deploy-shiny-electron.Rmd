...
UPDATE (2020-09-06): The approach described below is fragile and I no longer use it. Here are a couple of alternatives: [electricShine](https://chasemc.github.io/electricShine/) and [photon](https://github.com/ColumbusCollaboratory/photon).
...
2. Run `npm install -g electron-forge` in the Terminal (macOS)^[On macOS, you may need to provide permission to access `/usr/local/lib/node_modules`. [Solve the permission error](https://flaviocopes.com/npm-fix-missing-write-access-error/) by running `sudo chown -R $USER /usr/local/lib/node_modules` from the Terminal.] or Node.js command prompt (Windows) to install Electron Forge globally. 
...
2. Install `innoextract` with Chocolatey by running `choco install innoextract` from the command prompt with administrative privileges, i.e., right click on the command prompt and choose `Run as administrator`. `innoextract 1.8` is required. If you have a previous version of innoextract installed, then run `choco upgrade innoextract`.^[If that doesn't work, run `choco uninstall innoextract` and then run `choco install innoextract`.]
...
8. Run `npm start` in the Node.js command prompt to test if the app launches (and works correctly).^[One of the requirements is that `git` is installed. An installer is available [here](https://git-scm.com/download/win).]
