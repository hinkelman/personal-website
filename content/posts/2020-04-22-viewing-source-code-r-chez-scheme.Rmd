...
[`cfs.misc`](https://github.com/fishsciences/cfs.misc) is a small package with a few utility functions that are useful to our work at [Cramer Fish Sciences](https://www.fishsciences.net). One of the functions in `cfs.misc`, `water_year`, when given a date, returns the [water year](https://en.wikipedia.org/wiki/Water_year) (as defined by the USGS). After installing `cfs.misc`, you can view the source code by simply typing the function name in the console^[If you are using RStudio, you can also view source code with `View(cfs.misc::water_year)`, which opens a tab to show the source code and omits the bytecode information.] (here prefixed by the package name instead of loading the package via `library`).
